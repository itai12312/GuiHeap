...
public class Tester {
    ...
    public boolean testBinaryRep(){
        ...
        for(boolean bool:heapBinaryRep){
            ...
            System.out.println("err:size "+generatedHeap.size()+" "+generatedHeap.map.keySet().size()+" list size:"+this.list.size());
            String s=" ";
            if(heapBinaryRep ==null){
                s="no a valid heap";
            }else {
                for (boolean y : heapBinaryRep) {
                    if (y) {
                        s += "1 ";
                    } else {
                        s += "0 ";
    ...
    public void mixedTest(){
        ...
        for(int i=1;i<=20000;i++){
            ...
            while(generatedHeap.search(random)!=null || list.contains(random)){
            	random=rand.nextInt(Integer.MAX_VALUE);
            }
    ...
    public void generateHeap(int maxValue, int size, BinomialHeap heap, List<Integer> list){
        ...
        try{
            for(int i=0;i<size;i++){
                while(list.contains(random) || heap.search(random)!=null){
                	random=rand.nextInt(maxValue);
                }
                heap.insert(random);
                list.add(random);
    ...
    public void generateHeap23(int minValue,int maxValue, int size, BinomialHeap heap, List<Integer> list){
        Random rand=new Random();
        int random = 0;
        try{
    ...
    public void testMeld(int maxValue, int size){
        ...
        generateHeap23(100,1000, size, heap, list);
        System.out.println("heap1 "+heap.map.keySet().size()+" "+list.size());
        generateHeap23(1300,2000, size, this.generatedHeap, this.list);
        System.out.println("heap2 "+generatedHeap.map.keySet().size()+" "+this.list.size());
        try{
        ...
        System.out.println("2 heaps were melded!\nThe new (melded) heap has "+2*size+" elements "+this.generatedHeap.map.keySet().size());
        runTest();
        System.out.println("Success! :)");
    }
